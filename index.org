...
* General Settings
** Package managment
To extend Emacs we will want to be able to add external packages. Emacs comes with a built in package manager called ~package~ which we will use to get external package. Lets load it in using the ~require~ function and then initialize it.
...
The newer fancy way of managing package is using ~use-package~. I've included it as a git submodule in my ~.emacs.d~ directory which you can see in [[https://github.com/geokon-gh/.emacs.d][the repository]] (be sure to clone recursively if you decide to use my version). Or you can get yourself manually from: https://github.com/jwiegley/use-package

Once we have it in our directory we just need to add it to the ~load-path~ and load it in using the ~require~ function
#+BEGIN_SRC emacs-lisp :tangle init.el
  (setq
   load-path
   (cons
